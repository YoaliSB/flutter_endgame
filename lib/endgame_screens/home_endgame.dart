...
class ListInfinitySaga extends State<InfinitySaga>{
  ...
  Future<List<InfinityComic>> _getComics() async {
    ...
    var md5D = generateMd5("${now}");
    var url = "https://gateway.marvel.com/v1/public/series?title=infinity gauntlet"+
        "&ts=${now}"+
        "&apikey="+
        "&hash=${md5D}";
    print(url);
    ...
    for(var comic in results){
      ...
      InfinityComic infinityComic = InfinityComic(title, image);
